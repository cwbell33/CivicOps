...
// checkUrl: what the scheduled local refresh reads: one URL, or a list of URLs
//           for a source with several calendars. Only sources that have one are
//           checked. Some are data feeds rather than pages, and may contain
...
//           .ics feeds are read with scripts/read-calendar.mjs, which expands
//           repeating events correctly. Don't swap in Google Calendar's agenda
//           view: it files evening events under the next day for some calendars.
// Sources with no checkUrl have no calendar that can be read automatically.
const GCAL = "https://calendar.google.com/calendar/ical";

export const DEFAULT_SOURCES = [
  // No public events: search finds none, and its events tag is an archive ending in 2018.
  ...
  // Calendar loads with JavaScript and lists the same events as Complete Communities
  // Coalition, which Futurewise runs and which is checked below.
  { name: "Futurewise", group: "Housing & Urbanism", url: "https://futurewise.org/events/" },
  // Posts events to Eventbrite and Facebook, which both load with JavaScript.
  // Nothing upcoming as of September 2026.
  { name: "Share the Cities", group: "Housing & Urbanism", url: "https://sharethecities.org/events-resources" },
  // Replaces Seattle For Everyone, which has had no posts since 2021. Both are
  // Futurewise-run Seattle housing coalitions; this one is active.
  { name: "Complete Communities Coalition", group: "Housing & Urbanism", url: "https://www.completecommunitiescoalition.org/events", checkUrl: "https://www.completecommunitiescoalition.org/events" },
  ...
  // The Google Calendar embedded on its calendar page.
  { name: "Seattle Streets Alliance (formerly Seattle Neighborhood Greenways)", group: "Transit & Streets", url: "https://www.streetsalliance.org/get-involved/calendar/", checkUrl: `${GCAL}/lt0lmiorecihqc99od0hnrtl54%40group.calendar.google.com/public/basic.ics` },
  ...
  // Replaces VolunteerMatch, which now redirects to Idealist with mostly off-topic
  // results from outside King County. Lists volunteer work parties in Seattle parks.
  { name: "Green Seattle Partnership", group: "Volunteering", url: "https://seattle.greencitypartnerships.org/event/map/", checkUrl: "https://seattle.greencitypartnerships.org/event/map/" },
  ...
  // The Google Calendars embedded on its events page: its own, plus the imported
  // calendar of legislative district meetings. The embed also shows US holidays
  // and Snohomish County Democrats, listed separately below.
  { name: "King County Democrats", group: "Government & Political", url: "https://www.kcdems.org/events/", checkUrl: [
    `${GCAL}/c_a1c7abb132ed49adb762aee86f4397f8ab56715765cd4501f8c7b7d3a10b2336%40group.calendar.google.com/public/basic.ics`,
    `${GCAL}/i5kpik9o80ul47llj4nsl9asnu5rt5br%40import.calendar.google.com/public/basic.ics`,
  ] },
  { name: "Snohomish County Democrats", group: "Government & Political", url: "https://calendar.google.com/calendar/embed?src=snocodems%40gmail.com&ctz=America%2FLos_Angeles", checkUrl: `${GCAL}/snocodems%40gmail.com/public/basic.ics` },
